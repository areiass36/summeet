...
export class MeetingHandler implements StateHandler {
    ...
    onStateUpdated(state: State): void {
        ...
        for (let [id, onlinePlayer] of state.onlinePlayers) {
            const isOnVertical = Math.abs(onlinePlayer.position.x - localPlayer.position.x) <= 1 && onlinePlayer.position.y == localPlayer.position.y;
            const isOnHorizontal = onlinePlayer.position.x == localPlayer.position.x && Math.abs(onlinePlayer.position.y - localPlayer.position.y) <= 1;
